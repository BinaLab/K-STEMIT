"""Training entry point that crops a shared source dataset to smaller predict-count targets."""
...
DEFAULT_SOURCE_PREDICT_COUNT = 21


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Internal Ice Layer Multi Training With Shared dataset_predict_21")
    ...
    parser.add_argument("--predict-count", type=int, required=True, help="Target predict count to train on after cropping")
    parser.add_argument("--source-predict-count", type=int, default=DEFAULT_SOURCE_PREDICT_COUNT, help="Shared source dataset_predict_xx to load before cropping")
    parser.add_argument("--dataset-root", default=DEFAULT_DATASET_ROOT, help="Root directory containing dataset_predict_xx files")
    parser.add_argument("--source-dataset-path", default=None, help="Optional explicit path to the shared source dataset dill file")
    parser.add_argument("--split-file", default=None, help="Optional explicit split file path; defaults to the source dataset split file")
    parser.add_argument("--split-seed", type=int, default=1337, help="Seed used to create the shared split file")
    parser.add_argument("--split-count", type=int, default=5, help="Number of shuffled split permutations to save")
    parser.add_argument("--overwrite-splits", action="store_true", help="Regenerate the shared split file even if a matching one exists")
    return parser.parse_args()


def resolve_source_paths(args_cml: argparse.Namespace) -> tuple[str, str]:
    dataset_root = os.path.abspath(os.path.expanduser(args_cml.dataset_root))

    if args_cml.source_dataset_path is None:
        dataset_path = os.path.join(dataset_root, f"dataset_predict_{args_cml.source_predict_count}")
    else:
        dataset_path = os.path.abspath(os.path.expanduser(args_cml.source_dataset_path))

    if args_cml.split_file is None:
        split_file = os.path.join(dataset_root, f"splits_predict_{args_cml.source_predict_count}.npy")
    else:
        split_file = os.path.abspath(os.path.expanduser(args_cml.split_file))
...
def validate_source_dataset(dataset, dataset_path, target_predict_count, ablation_enabled):
    if len(dataset) == 0 or len(dataset[0]) == 0:
        raise ValueError(f"Source dataset is empty or malformed: {dataset_path}")

    source_feature_count = dataset[0][0].x.shape[1]
    source_target_count = dataset[0][0].y.shape[1]
    expected_feature_count = 10 if ablation_enabled else 3

    if source_feature_count != expected_feature_count:
        raise ValueError(
            "Dataset feature count mismatch for "
            f"{dataset_path}: found {source_feature_count}, expected {expected_feature_count}. "
            "Rebuild the shared source dataset with the matching physical-feature setting before training."
        )

    if source_target_count < target_predict_count:
        raise ValueError(
            f"Source dataset target width is {source_target_count}, which is smaller than requested predict count {target_predict_count}."
        )

    return source_feature_count, source_target_count


def crop_graph_collection_targets(graph_collection, predict_count):
    for graph in graph_collection:
        if hasattr(graph, "y") and graph.y is not None:
            graph.y = graph.y[:, :predict_count].clone()


def crop_dataset_targets(dataset_split, predict_count):
    for graph_collection in dataset_split:
        crop_graph_collection_targets(graph_collection, predict_count)
...
def main():
    ...
    dataset_path, split_file = resolve_source_paths(args_cml)
    if not os.path.isfile(dataset_path):
        raise FileNotFoundError(f"Source dataset file not found: {dataset_path}")

    dataset = load_dill(dataset_path)
    source_feature_count, source_target_count = validate_source_dataset(
        dataset,
        dataset_path,
        args_cml.predict_count,
        args_cml.ablation.lower() == "true",
    )
    ...
    accelerator.print("Source Dataset Path:", dataset_path)
    accelerator.print("Dataset Samples:", len(dataset))
    accelerator.print("Source Predict Count:", source_target_count)
    accelerator.print("Target Predict Count:", args_cml.predict_count)
    accelerator.print("Feature Count:", source_feature_count)
    accelerator.print("Shared Split File:", split_file)
    ...
    for split_idx in range(args_cml.split_count):
        ...
        crop_dataset_targets(training_dataset, args_cml.predict_count)
        crop_dataset_targets(val_dataset, args_cml.predict_count)
        crop_dataset_targets(testing_dataset, args_cml.predict_count)
